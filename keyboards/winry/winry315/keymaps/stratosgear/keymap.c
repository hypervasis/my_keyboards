...
// Keycodes that are emitted whenever the right encoder is rotated to indicate the layer
// selected
uint16_t layer_emit_keys[] = {KC_F11, KC_F12};

// clang-format off
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    // Base
    [_BASE] = LAYOUT_top(
               KC_HOME, KC_F11, CK_OBS_CAMERA,
    ...
    [_OBS] = LAYOUT_top(
                      KC_HOME,         KC_F12,          KC_TRNS,
        C(KC_F12),    C(S(KC_F12)),    C(G(KC_F12)),    C(S(G(KC_F12))),    S(KC_F12),
        A(KC_F12),    A(S(KC_F12)),    A(G(KC_F12)),    A(S(G(KC_F12))),    G(KC_F12),
        C(A(KC_F12)), C(A(S(KC_F12))), C(A(G(KC_F12))), C(A(S(G(KC_F12)))), QK_BOOT
    ),
...
uint16_t obs_scene_keycodes[] = {C(G(KC_F12)), C(A(G(KC_F12))), C(A(KC_F12)),
                                 C(KC_F12)};
